...
import static com.sundy.sundyutils.MyApplication.myApp;
...
public final class ApkUtil {
    ...
    public static String getVersionName() {
        ...
        try {
            ...
            packInfo = packageManager.getPackageInfo(myApp.getPackageName(), 0);
    ...
    public static String getAppName() {
        try {
            PackageManager packageManager = myApp.getPackageManager();
            PackageInfo packageInfo = packageManager.getPackageInfo(myApp.getPackageName(), 0);
            int labelRes = packageInfo.applicationInfo.labelRes;
            return myApp.getResources().getString(labelRes);
        } catch (NameNotFoundException e) {
            e.printStackTrace();
        }
        return null;
...
//            return false;
//        try {
//            myApp.getPackageManager().getApplicationInfo(packageName, PackageManager.GET_UNINSTALLED_PACKAGES);
//            return true;
//        } catch (NameNotFoundException e) {
//            return false;
//        }
//    }
    ...
    public static void restart() {
        Intent i = myApp.getPackageManager().getLaunchIntentForPackage(myApp.getPackageName());
    ...
    public static boolean startApp(String packageName) {
        Intent LaunchIntent = myApp.getPackageManager().getLaunchIntentForPackage(packageName);
        if (LaunchIntent == null) {
            return false;
        } else {
            myApp.startActivity(LaunchIntent);
            return true;
        }
    }
    ...
    private static boolean isShortCutExist(String appName) {
        ...
        Cursor c = MyApplication.myApp.getContentResolver().query(uri, null, selection,
                selectionArgs, null);
        if (c == null) {
            return false;
        }
    ...
    public static boolean isStorageFull() {
        int mb = 1024 * 1024;
        double totalSize = MemorySpaceUtil.getInstance().getSysTotalSize() / mb;
        double systemSurplus = MemorySpaceUtil.getInstance().getSystemAvailableSize() / mb;
